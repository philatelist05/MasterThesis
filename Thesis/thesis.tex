...
\begin{enumerate}
		...
		\item Software Tools and Applications
\end{enumerate}
...
The \textit{Lexical Database} forms the heart of the WordNet system. It is the result of a processing all lexical source files. Unfortunately the tools to access the database are quite outdated. They are written in ancient programming languages like Perl or awk, however there also exists an online query interface\footnote{\url{http://wordnetweb.princeton.edu/perl/webwn} accessed 2018/06/14}. The database uses several indexes to increase the performance of search queries, each serving a dedicated purpose. 

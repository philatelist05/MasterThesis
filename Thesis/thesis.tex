...
Based on existing efforts for ontology validation using crowdsourcing~(see section~\ref{sec:ucomp_protege_plugin}), we formulate the following \textbf{evaluation hypothesis}:
\begin{quotation}
...
\end{figure}
In crwowsourcing contexts this means that yes-questions are either correctly answered~(TP) or incorrectly~(FN) and no-questions are either correctly answered~(TN) or incorrectly~(FP).

\paragraph{Precision} Precision is interpreted as the ratio of correctly answered yes-questions~(TP) over the total number of answered yes-questions:
\[ Precision = \frac{TP}{TP + FP} \]
For concept relevance, values of precision close to $1.0$ show that the crowd correctly predicts relevant concepts but maybe fails at rejecting irrelevant concepts. 
\paragraph{Recall}
...
%MEETING MINUTES:
%Evaluation Setup:
%	1.) Define the goal for evaluation --> So the Inter-rater Agreement increase, and precision and recall increase
%			*) Depends on the actual data if these goals are feasible/reachable. 
%	2.) Data --> What Ontologies can be used as a source for evaluation (O1, O2, ...)
%	3.) Setup --> How the actual test setup look like.
%			*) Should we use the very same ontology for all context enrichment methods? 
%				--> Then the results would be of general applicability (preferable solution)
%			*) Or could we use different learned ontologies (e.g. finance/climate) or generic ontologies (e.g. pizza) 
%			   for specific context enrichment methods (e.g. concept metadata extraction on Biomed ontology)
%	4.) Measures/Metrics --> What Metrics to use?
%		*) Precision (needs baseline --> e.g. a correct, golden standard ontology)
%		*) Recall (needs baseline --> e.g. a correct, golden standard ontology)
%		*) Inter-rater Agreement
%		*) a combination of the above / all of the above

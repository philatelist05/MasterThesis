...
To get different view of the overall crowd worker performance, \hyperref[fig:hist_level_finance_all]{Figure~\ref*{fig:hist_level_finance_all}} 
shows bar plots of the performance levels. Each level combines the agreement ratio with the amount of correct/incorrect judgements, yielding a higher score when most contributors agreed on correct answers and a lower score when they disagreed or answer incorrectly.
A common phenomena of all approaches was the high level of correct answered where most contributor agreed. Indeed, this holds for the other datasets too with minor differences. However, after analysing the concepts that were accepted and those that were declined, this is rather related to the generic nature of the used datasets. For example, whereas a positive judgement of the concept \emph{budget} for the finance domain is comparatively easy even if no additional information was provided, judging the concept \emph{world} is much more challenging. Judging generic concepts is therefore is probably done better by domain experts who share a common understanding of the used vocabulary. 
...
The summary statistics in~\hyperref[table:level_corr_incorr_finance]{Table~\ref*{table:level_corr_incorr_finance}} manifest the observations made so far. It shows the statistics of each method ranked by mean value. Judging the worker performance based on agreement level, the order is no different than ranked by F-Measure as in~\hyperref[table:bench_p_r_f_finance]{Table~\ref*{table:bench_p_r_f_finance}}. 

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Based on the metrics discussed in \hyperref[sec:evaluation_metrics]{Section~\ref*{sec:evaluation_metrics}}, the results of the crowdsourced ontology
validation jobs are presented. 
...
In this section results from the crowdsourced ontology validation in the field of climate change are presented. The ontology used as a baseline for all calculations in this section is introduced in detail in \hyperref[sec:evaluation_datasets]{Section~\ref*{sec:evaluation_datasets}}. 

The results of the benchmark are presented in~\hyperref[table:bench_p_r_f]{Table~\ref*{table:bench_p_r_f}}. For comparison we also performed ontology validation without any of the discussed context enrichment methods~(indicated as \emph{None}). Given the relatively small set number concepts, any of the context enrichment method perform better than having no context at all. Surprisingly, in terms of recall the contrary holds. Indeed, crowd workers tend to positively answer questions in case of uncertainty and no additional information other than the concept name is present. 
\begingroup
\renewcommand{\arraystretch}{1.5}
\begin{table}
	\begin{tabularx}{\textwidth}{l c*{3}{Y}}
...
We also measured the average agreement ratio~(inter-rater agreement) in this dataset. \hyperref[fig:hist_agreement_all]{Figure~\ref*{fig:hist_agreement_all}} shows the distribution of the agreement ratio among all validated concepts. As we required 5 judgements for every concept, giving $5/0$, $4/1$ or $3/2$ levels of agreement which equates to an agreement ratio of $1.0$, $0.8$ and $0.6$ respectively. 
...
The highest agreement exhibits \emph{External~Source}, followed by \emph{None}. This is somewhat interesting as these are the methods with the lowest performance with regard to F-measure. In fact, the agreement ratio just describes to what extent the responses of crowd workers coincide. From the observations in this dataset~(and among all experiments), it is hard if not impossible to draw conclusions exclusively based on agreement. In fact, when looking closely at the judgements with the highest agreement~ratio~($1.0$) among the incorrect answers, $16$ of $17$ judgements for External~Source and $3$ of $6$ judgements for Neighbouring~Nodes had context added, indicating that in this case crowd workers agreed on incorrect values even though that concept descriptions were added. 

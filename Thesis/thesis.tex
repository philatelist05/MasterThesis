...
In this Section we give an overview of existing approaches for embedding metadata in ontologies and then describe the approach we adopted to extract metadata from selected vocabularies.



%Start with motivation from ontology authoring (e.g. why descriptions are important)
%Give some good examples with added descriptions (e.g. medical ontology which already has some definitions; collaboratively edited;)
%List options for manual provisioning of context
%	(e.g. by experts, at creation time, continuously in mainentance mode)
%	!!Mention SKOS, Dublin-Core --> method of adding metadata to the ontology!!

%Enrichment process: Manually add descriptions based on expert agreement
